...
def chromatic_removal(I_in, L_hor=7, L_ver=4, rho=(-0.25, 1.375, -0.125), tau=15, gamma_1=128, gamma_2=64, use_yuv=False):
    R_in, G_in, B_in = I_in[..., 0], I_in[..., 1], I_in[..., 2]
    ...
    K_r_ver, K_b_ver, K_rTI_ver, K_bTI_ver, R_max_ver, B_max_ver, R_min_ver, B_min_ver = (
        K_r_ver.T, K_b_ver.T, K_rTI_ver.T, K_bTI_ver.T, R_max_ver.T, B_max_ver.T, R_min_ver.T, B_min_ver.T
    )

    K_b = np.where(np.abs(K_b_hor) < np.abs(K_b_ver), K_b_ver, K_b_hor)
    K_r = np.where(np.abs(K_r_hor) < np.abs(K_r_ver), K_r_ver, K_r_hor)

    K_bTI = np.where(np.abs(K_bTI_hor) < np.abs(K_bTI_ver), K_bTI_ver, K_bTI_hor)
    B_max, B_min = np.where(np.abs(K_bTI_hor) < np.abs(K_bTI_ver), (B_max_ver, B_min_ver), (B_max_hor, B_min_hor))

    K_rTI = np.where(np.abs(K_rTI_hor) < np.abs(K_rTI_ver), K_rTI_ver, K_rTI_hor)
    R_max, R_min = np.where(np.abs(K_rTI_hor) < np.abs(K_rTI_ver), (R_max_ver, R_min_ver), (R_max_hor, R_min_hor))
...
def ti_and_ca_filtering1D(X_in, G_in, Y_in, L, rho, alpha_X, tau):
    LL = 2 * L + 1
    X_in_padded = np.pad(X_in, [(0, 0), (L, L)], mode='edge')
    G_in_padded = np.pad(G_in, [(0, 0), (L, L)], mode='edge')
    Y_in_padded = np.pad(Y_in, [(0, 0), (L, L)], mode='edge')
    grad_X = np.pad(np.diff(X_in_padded, 1, axis=-1), [(0, 0), (0, 1)], mode='edge')
    ...
    for j in tqdm(range(N)):
        ...
        grad_G_L = grad_X[:, j:j + LL]  # (M, 2L+1)

        X_in_L_max, X_in_L_min = compute_local_max_and_min(X_in_L)

        mask = X_in_L[:, L] > G_in_L[:, L]
        mask_not = ~mask

        X_pf = rho[0] * X_in_L_max + rho[1] * X_in_L + rho[2] * X_in_L_min
        X_pf[mask_not] = rho[0] * X_in_L_min[mask_not] + rho[1] * X_in_L[mask_not] + rho[2] * X_in_L_max[mask_not]

        X_TImin = np.maximum(X_in_L_min, G_in_L)
        X_TImax = np.minimum(X_in_L_max, G_in_L)

        X_TI = np.clip(X_pf, a_min=X_TImin, a_max=X_TImax)

        K_TI_hor[:, j] = X_TI[:, L] - G_in_L[:, L]
        X_max[:, j] = np.squeeze(X_in_L_max, 1)
        X_min[:, j] = np.squeeze(X_in_L_min, 1)

        K_TI = X_TI - G_in_L
        S_K = compute_S_K(K_TI, tau)
        w_K = compute_w_K(K_TI, grad_X=grad_X_L, grad_G=grad_X_L, Y=Y_in_L, alpha_X=alpha_X)
        W_K = S_K * w_K

        W_K /= np.sum(W_K, axis=-1, keepdims=True)
        K_hor[:, j] = np.einsum('ij,ij->i', W_K, clip(K_TI, K_TI[..., L]))
...
def transiant_improvement1D(X_in, G_in, L, rho):
    ...
    for j in range(N):
        ...
        ## Compute min and max on windows
        ...
        ## Restricting the range of admissible values
        ...
        ## Clipping the filtered imaged in the admissible set on values
...
def compute_local_max_and_min_color_constraints(X, G, beta):
    ## Recompute updated local max and min 
    LL = X.shape[-1]  # (M, 2L+1)
    L = (LL-1) // 2
